...
## Task 6: Create `.github/workflows/terraform-deploy.yml`
    ...
    inputs:
      action:
        ...
        options:
          ...
          - apply
      scope:
        description: "Which layers to target"
        required: true
        type: choice
        options:
          - all
...
env:
  TF_VAR_FILE: "tfvars/${{ inputs.client }}/${{ inputs.environment }}.tfvars"
  TF_VERSION: "1.14.6"
...
jobs:
  ...
  # =========================================================================
  networking:
    name: "Networking (${{ inputs.action }})"
    if: contains(fromJSON('["all","networking"]'), inputs.scope)
    ...
    steps:
      ...
      - name: Terraform Init
        run: terraform -chdir=gob/networking init -backend-config="prefix=${{ inputs.client }}/${{ inputs.environment }}/networking"

      - name: Terraform Plan
        run: terraform -chdir=gob/networking plan -var-file=${{ env.TF_VAR_FILE }}

      - name: Terraform Apply
        if: inputs.action == 'apply'
        run: terraform -chdir=gob/networking apply -auto-approve -var-file=${{ env.TF_VAR_FILE }}

  # =========================================================================
  # Layer 2: Database
  # =========================================================================
  database:
    name: "Database (${{ inputs.action }})"
    needs: [networking]
    if: |
      always() &&
    ...
    steps:
      ...
      - name: Terraform Init
        run: terraform -chdir=gob/database init -backend-config="prefix=${{ inputs.client }}/${{ inputs.environment }}/database"

      - name: Terraform Plan
        run: terraform -chdir=gob/database plan -var-file=${{ env.TF_VAR_FILE }}

      - name: Terraform Apply
        if: inputs.action == 'apply'
        run: terraform -chdir=gob/database apply -auto-approve -var-file=${{ env.TF_VAR_FILE }}

  # =========================================================================
  # Layer 3: Compute
  # =========================================================================
  compute:
    name: "Compute (${{ inputs.action }})"
    needs: [database]
    if: |
      always() &&
      contains(fromJSON('["all","compute"]'), inputs.scope) &&
    ...
    steps:
      ...
      - name: Terraform Init
        run: terraform -chdir=gob/compute init -backend-config="prefix=${{ inputs.client }}/${{ inputs.environment }}/compute"

      - name: Terraform Plan
        run: terraform -chdir=gob/compute plan -var-file=${{ env.TF_VAR_FILE }}

      - name: Terraform Apply
        if: inputs.action == 'apply'
        run: terraform -chdir=gob/compute apply -auto-approve -var-file=${{ env.TF_VAR_FILE }}
```
...
git commit -m "feat(automation): add terraform-deploy GitHub Actions workflow"
```

---

## Task 7: Create `.github/workflows/terraform-destroy.yml`

**Files:**
- Create: `.github/workflows/terraform-destroy.yml`

**Step 1: Create the destroy workflow**
...
on:
  workflow_dispatch:
    inputs:
      scope:
        description: "Which layers to destroy"
        required: true
        type: choice
        options:
          - all
...
jobs:
  # =========================================================================
  # Layer 3: Compute (destroyed first)
  # =========================================================================
  compute:
    name: "Compute (destroy)"
    if: contains(fromJSON('["all","compute"]'), inputs.scope)
    runs-on: ubuntu-latest
    steps:
      - name: Checkout
        uses: actions/checkout@11bd71901bbe5b1630ceea73d27597364c9af683 # v4.2.2
      ...
      - name: Terraform Destroy
        run: terraform -chdir=gob/compute destroy -auto-approve -var-file=${{ env.TF_VAR_FILE }}

  # =========================================================================
  # Layer 2: Database (destroyed second)
  # =========================================================================
  database:
    ...
    if: |
      always() &&
      contains(fromJSON('["all","database"]'), inputs.scope) &&
      (needs.compute.result == 'success' || needs.compute.result == 'skipped')
    runs-on: ubuntu-latest
    steps:
      - name: Checkout
        uses: actions/checkout@11bd71901bbe5b1630ceea73d27597364c9af683 # v4.2.2
      ...
      - name: Terraform Destroy
        run: terraform -chdir=gob/database destroy -auto-approve -var-file=${{ env.TF_VAR_FILE }}

  # =========================================================================
  # Layer 1: Networking (destroyed last)
  # =========================================================================
  networking:
    ...
    if: |
      always() &&
      contains(fromJSON('["all","networking"]'), inputs.scope) &&
      (needs.database.result == 'success' || needs.database.result == 'skipped')
    runs-on: ubuntu-latest
    steps:
      - name: Checkout
      ...
      - name: Terraform Destroy
        run: terraform -chdir=gob/networking destroy -auto-approve -var-file=${{ env.TF_VAR_FILE }}
```
...
git commit -m "feat(automation): add terraform-destroy GitHub Actions workflow"
```
